...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/08 16:50:39 by andrferr          #+#    #+#             */
/*   Updated: 2022/11/09 10:53:49 by andrferr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_line(char *buffer)
{
	...
	while (buffer[i] != '\n' && buffer[i] != '\0')
		i++;
	str = allocate_at_zero(i + 2, sizeof(char));
	if (!str)
		return (NULL);
	i = 0;
	while (buffer[i] != '\n' && buffer[i] != '\0')
...
char	*read_file(int fd, char *buffer)
{
	...
	if (!buffer)
		buffer = allocate_at_zero(1, sizeof(char));
	if (!buffer)
		return (NULL);
	ret = 1;
	while (ret > 0)
